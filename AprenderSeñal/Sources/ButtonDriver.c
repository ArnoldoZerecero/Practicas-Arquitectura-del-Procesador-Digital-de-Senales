...
#define NUMERO_BOTONES 1

const unsigned short kwaButtonArray[NUMERO_BOTONES] = {(1<<0)}; /*Arreglo de pines donde se encuentran los botones*/
...
void vfnButtonInit(void)
{
	...
	GPIOB_PDIR |= kwaButtonArray[0]; /*Se asegura que los botones comiencen apagados*/
}
...
{	
	if (bBoton == 0) /*Por protección, si el usuario introduce un pin que no tiene botón*/
	{
...
void vfnButtonDriver(void) /*Driver que revisa los valores de los botones y realiza el debouncing en cada ciclo del big main loop*/
{
	...
	unsigned char baButtonRead[4]; /*Arreglo para guardar y comparar los estados anteriores del botón*/
