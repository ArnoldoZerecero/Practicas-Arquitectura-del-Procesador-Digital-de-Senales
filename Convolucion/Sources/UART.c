...
void vfnUartTransmision(unsigned char * bpDataToSend, unsigned short wDataSize) /*Función de transmisión, recibe el apuntador al arreglo de datos y el número de datos a transmitir*/
{
	if(!(UART0_C2 & UART0_C2_TIE_MASK)) /*Si la interrupción para el transmisor no está activada (aun no se comienza a mandar dato alguno)*/
	{
		...
		gwIndex = 0; /*Cada que se realice una nueva transmisión, se reinicia el índice en 0*/
		
		UART0_D = gbpDataToSend[gwIndex]; /*Se escribe en UART0_D el primer dato*/
		gwIndex++; /*Se incrementa el índice*/

		UART0_C2 |= (UART0_C2_TIE_MASK | UART0_C2_TE_MASK); /*Se activa el interrupt del transmisor y el enable del transmisor al comenzar a mandar datos, para no volver a caer en la condición hasta mandar la trama completa*/
	}
}

void UART0_IRQHandler(void) /*Función de interrupción. Se entra aquí cada que las banderas de Transmission Buffer Empty y Receiver Buffer Full se activan, para escribir un nuevo dato en el transmisor y leerlo en el receptor. Se comparten variables globales*/
{
